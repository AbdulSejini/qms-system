...
export default function AuditsPage() {
  ...
  // Audits data
  const [auditsData, setAuditsData] = useState<Audit[]>(initialAudits);
  ...
  useEffect(() => {
    ...
    if (storedAudits.length > 0) {
      // Merge localStorage audits with initial audits, avoiding duplicates
      setAuditsData(prev => {
        const existingIds = new Set(prev.map(a => a.id));
        const newAudits = storedAudits.filter((a: Audit) => !existingIds.has(a.id));
        return [...newAudits, ...prev];
      });
    }
  }, []);
